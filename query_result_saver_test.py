...
class TestQueryResultSaver(unittest.TestCase):
    def setUp(self):
        ...
        with open(SAVE_PATH + '/test_bigquery.csv', newline='') as bq_csv:
            self.result_bigquery = list(csv.reader(
                bq_csv, delimiter=' ', quotechar='|'))
        with open(SAVE_PATH + '/test_mysql.csv', newline='') as mysql_csv:
            self.result_mysql = list(csv.reader(
                mysql_csv, delimiter=' ', quotechar='|'))

    def test_result_to_csv_with_bigquery(self):
        with AdapterBigquery(SERVICE_ACC) as adapter:
            QueryResultSaver(query_bigquery, adapter).result_to_csv(
                self.save_path)
        with open(self.save_path, newline='') as result_csv:
            result = csv.reader(result_csv, delimiter=' ', quotechar='|')
            self.assertEqual(list(result), self.result_bigquery)

    def test_result_to_csv_with_mysql(self):
        with AdapterMysql(DB_CONFIG) as adapter:
            QueryResultSaver(query_mysql, adapter).result_to_csv(
                self.save_path)
        with open(self.save_path, newline='') as result_csv:
            result = csv.reader(result_csv, delimiter=' ', quotechar='|')
            self.assertEqual(list(result), self.result_mysql)
